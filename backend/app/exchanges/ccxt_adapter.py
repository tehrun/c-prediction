...
class CcxtExchangeAdapter(ExchangeAdapter):
    ...
    async def load_markets(self) -> list[ExchangeMarket]:
        ...
        out = []
        for m in raw.values():
            limits = m.get("limits", {}); precision = m.get("precision", {})
            out.append(ExchangeMarket(symbol=m["symbol"], base=m.get("base") or "", quote=m.get("quote") or "", active=bool(m.get("active", True)), spot=bool(m.get("spot", False)), amount_precision=precision.get("amount"), price_precision=precision.get("price"), minimum_amount=dec(limits.get("amount", {}).get("min")), minimum_cost=dec(limits.get("cost", {}).get("min"))))
        return out
    async def fetch_ohlcv(self, symbol: str, timeframe: str, since: datetime | None = None, limit: int | None = None) -> list[Candle]:
        since_ms = int(since.timestamp() * 1000) if since else None
        rows = await self.client.fetch_ohlcv(symbol, timeframe=timeframe, since=since_ms, limit=limit)
        return [Candle(opened_at=datetime.fromtimestamp(r[0]/1000, UTC), open=Decimal(str(r[1])), high=Decimal(str(r[2])), low=Decimal(str(r[3])), close=Decimal(str(r[4])), volume=Decimal(str(r[5]))) for r in rows]
    async def fetch_ticker(self, symbol: str) -> Ticker:
        ...
        return Ticker(symbol=symbol, last=dec(t.get("last")), quote_volume=dec(t.get("quoteVolume")), timestamp=ts)
...
class KrakenExchangeAdapter(CcxtExchangeAdapter):
    key = "kraken"; ccxt_id = "kraken"
class BinanceExchangeAdapter(CcxtExchangeAdapter):
    key = "binance"; ccxt_id = "binance"
