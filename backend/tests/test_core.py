...
def test_settings_defaults():
    assert Settings().market_universe_max == 20


def test_factory():
    assert get_exchange_adapter("kraken").key == "kraken"
...
def test_stablecoin_and_leverage_filters():
    ms = [
        ...
        ExchangeMarket(
            ...
            price_precision=2,
            minimum_amount=Decimal("1"),
            quote_volume=Decimal("9"),
        ),
    ]
    assert [m.symbol for m in filter_market_universe(ms)] == ["BTC/USD"]
    assert is_leveraged_token("ETH3L")
...
def test_missing_detection():
    ...
    assert expected_missing(
        [s, s + timedelta(hours=2)], s, s + timedelta(hours=2), timedelta(hours=1)
    ) == [s + timedelta(hours=1)]
